...
import { NoContentsStyle, ContentsStyle, CardContentStyle, TypographyStyle, TitleLink } from './CourseList.css';

function CourseList(props: any) {
  const { courses } = props;
  return (
    <div className={ContentsStyle}>
      {(courses === null || JSON.stringify(courses) === '{}') && <div className={NoContentsStyle}>No Contents</div>}
      {!(courses === null || JSON.stringify(courses) === '{}') &&
        courses.map((myContent: Course) => (
